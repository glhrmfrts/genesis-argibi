...
void updateCamera() {
	...
	cameraPosition.x = clamp(cameraPosition.x, 0, mapSize.max.x); // 768 - 320 = 448 (level width - screen width)
	cameraPosition.y = clamp(cameraPosition.y, 0, mapSize.max.y); // 768 - 224 = 544 (level height - screen height)
